  ...
  return <section className="calendar-section" aria-labelledby="calendar-title" onPointerDown={beginCalendarSwipe} onPointerMove={trackCalendarSwipe} onPointerUp={finishCalendarSwipe} onPointerCancel={cancelCalendarSwipe} onClickCapture={suppressSwipeClick}>
    ...
    {!loading && !error ? (mode === "week" ? <>
      ...
      {dates.map((date) => {
        ...
        const weatherBadge = (variant: "desktop" | "mobile") => weather === undefined ? null : <span className={`month-day-weather is-${variant}`} aria-label={`天气：${weather.text}，${weather.tempMin}到${weather.tempMax}摄氏度`} title={`天气：${weather.text}，${weather.tempMin}~${weather.tempMax}°C`}><span aria-hidden="true">{getWeatherEmoji(weather.icon)}</span></span>;
