...
type SettingsPageId = "account/session" | "data/import-export" | "data/backup" | "data/demo" | "data/photos" | "appearance/interface" | "integrations/weather" | "integrations/ai" | "integrations/movie" | "private/cycle" | "about";
...
const AI_ASSISTANT_VISIBLE_STORAGE_KEY = "lifeos.ai.assistant-visible";

const SETTINGS_PAGE_GROUPS: readonly { readonly id: string; readonly label: string; readonly pages: readonly { readonly id: SettingsPageId; readonly label: string }[] }[] = [
...
const SETTINGS_PAGE_IDS = new Set<SettingsPageId>(SETTINGS_PAGE_GROUPS.flatMap((group) => group.pages.map((page) => page.id)));
const DEFAULT_SETTINGS_PAGE: SettingsPageId = "account/session";

function readSettingsPageFromHash(): SettingsPageId {
...
function settingsHash(page: SettingsPageId): string { return `#settings/${page}`; }

function readAssistantVisibility(): boolean {
  try { return window.localStorage.getItem(AI_ASSISTANT_VISIBLE_STORAGE_KEY) !== "0"; } catch { return true; }
}
/**
...
const MOBILE_NAV_ITEMS: readonly { id: AppView; label: string; icon: LucideIcon }[] = [
  { id: "today", label: "今天", icon: Sun },
  { id: "calendar", label: "日历", icon: CalendarRange },
  { id: "notes", label: "笔记", icon: NotebookPen },
...
const MOBILE_MORE_ITEMS: readonly { id: AppView; label: string; icon: LucideIcon }[] = [
  { id: "timeline", label: "时间轴", icon: CalendarDays },
  { id: "tasks", label: "任务", icon: ListChecks },
  { id: "timemachine", label: "时光机", icon: History },
  SETTINGS_NAV_ITEM,
    ...
    <div className="composer-footer">
      <div className="composer-fields">
        ...
        <DateField value={occurredAt} onChange={onOccurredAtChange} label="发生时间" showTime capped />
          {kind === "task" ? <DateField value={dueAt} onChange={onDueAtChange} label="截止时间" showTime /> : null}        {isBackfillDate ? <label className={`backfill-toggle ${isBackfill ? "is-on" : ""}`} title="将这条记录标记为补记"><input type="checkbox" checked={isBackfill} onChange={(event) => onBackfillChange(event.target.checked)} /><History size={14} strokeWidth={1.9} aria-hidden="true" /><span>补记</span></label> : null}
...
function MobileMenuDialog({ open, activeView, onClose, onNavigate, onOpenSearch }: { open: boolean; activeView: AppView; onClose: () => void; onNavigate: (view: AppView) => void; onOpenSearch: () => void }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
...
function AiSettingsCard({ status, open = false, assistantVisible, onAssistantVisibleChange, onChanged }: { readonly status: AiStatus; readonly open?: boolean; readonly assistantVisible: boolean; readonly onAssistantVisibleChange: (visible: boolean) => void; readonly onChanged: (status: AiStatus) => void }) {
...
function CycleSettingsCard({ module, onSaveConfig }: { readonly module: CycleIntimacyModuleData | null; readonly onSaveConfig: (config: CycleIntimacyModuleConfig) => Promise<void> }) {
...
function SettingsView({ page, onNavigatePage, onImport, onLogout, logoutBusy, authRequired, aiStatus, onAiStatusChange, assistantVisible, onAssistantVisibleChange, backupStatus, backupBusy, onBackup, onBackupStatusChange, weatherStatus, weatherProfiles, weatherActiveProfileId, onWeatherStatusChange, onWeatherProfilesChange, movieStatus, onMovieStatusChange, demoCount, hideDemo, demoBusy, demoDeleteArmed, onToggleDemo, onDeleteDemo, uiFont, onUiFontChange, onAssetsChanged, cycleModule, onSaveCycleConfig }: { page: SettingsPageId; onNavigatePage: (page: SettingsPageId) => void; onImport: () => void; onLogout: () => void; logoutBusy: boolean; authRequired: boolean; aiStatus: AiStatus; onAiStatusChange: (status: AiStatus) => void; assistantVisible: boolean; onAssistantVisibleChange: (visible: boolean) => void; backupStatus: BackupStatus; backupBusy: boolean; onBackup: (action: "local" | "s3" | "test" | "dual") => void; onBackupStatusChange: (status: BackupStatus) => void; weatherStatus: WeatherStatus | null; weatherProfiles: readonly WeatherProfile[]; weatherActiveProfileId: string | null; onWeatherStatusChange: (status: WeatherStatus) => void; onWeatherProfilesChange: (payload: WeatherProfilesResponse) => void; movieStatus: MovieModuleStatus; onMovieStatusChange: (status: MovieModuleStatus) => void; demoCount: number; hideDemo: boolean; demoBusy: boolean; demoDeleteArmed: boolean; onToggleDemo: () => void; onDeleteDemo: () => void; uiFont: UiFontId; onUiFontChange: (value: UiFontId) => void; onAssetsChanged: () => void; cycleModule: CycleIntimacyModuleData | null; onSaveCycleConfig: (config: CycleIntimacyModuleConfig) => Promise<void> }) {
  ...
  const pageContent = page === "account/session" ? <div className="settings-card settings-account-card"><div className="settings-card-icon"><LockKeyhole size={18} aria-hidden="true" /></div><div className="settings-card-copy"><strong>{authRequired ? "已登录" : "本机访问"}</strong><small>{authRequired ? "当前会话受访问密码保护。" : "当前实例未启用登录密码。"}</small></div>{authRequired ? <button className="danger-button settings-action" type="button" onClick={onLogout} disabled={logoutBusy}>{logoutBusy ? <LoaderCircle className="spin" size={16} aria-hidden="true" /> : <LogOut size={16} aria-hidden="true" />}<span>{logoutBusy ? "退出中" : "退出登录"}</span></button> : null}</div>
    ...
    : page === "integrations/movie" ? <MovieSettingsCard status={movieStatus} onChanged={onMovieStatusChange} />
