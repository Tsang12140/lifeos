...
- **`reapChromeSync` 返回 true ≠ 目录真没了**：09-22 每轮必现 —— 删除成功（2.75s 采样全 false），几秒后目录**又完整长回来**，而事后 `chrome.exe` = 0；**每轮漏约 50MB**，根因未查清（`docs/todo.md` §12）→ 收尾要**自己列一遍 profile 目录**再报数。
...
- **UI 硬规则**（detail `## 记录卡片与 grid` / `## 天气天空` / `## 动画铁律` / `## 布局稳定性` / `## 全站不可选中`）：卡片宽 **430.08px**、只有高度随内容（`width` 与 `max-width:100%` 都要）；**双列已删不要恢复**；哨兵 = `verify-photo-grid` 的 `five lines`。几何恒定的元素一律显式定位（grid 自动排布会「只在部分记录上错」）。**不许移动的细周期图案**（摩尔纹）。`animation-fill-mode:none` + 正延迟要配屏幕外基准位；`html{scrollbar-gutter:stable}`；CSS `auto` 高度不可插值。**界面文字一律不可选中**（`body`/`button` 默认禁 + 白名单**只放承载文字的叶子**，清单看 `styles.css` 顶部那块；例外只有 `.settings-ai-effective-item strong`），验收 `.review/verify-selectability.mjs`（**只读贴 5199**，三层判据）。
