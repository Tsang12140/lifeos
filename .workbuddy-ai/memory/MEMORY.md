...
## 天气位置下拉：完整草稿已存在，但**从未集成**（2026-09-18 查明）

- 主人 09-17 要求「和风国内地区做成下拉、映射 Location ID」（因为手输城市搜不到）。
  **某 AI 已做完并自测通过**，但当时判断「另一个 AI 正在改项目」→ **只写草稿、未动任何现有文件**，等主人发话。
- 位置：`.review/weather-location-draft/`（`INTEGRATION-PLAN.md` 含逐文件 diff）。产物 **34 省 / 392 市 / 3572 位置 / 155KB**，官方源 v202506200。
- **2026-09-18 复验**：`verify-data.mjs` 33/0、`verify-lookup.mjs` 30/0（3572 位置全量往返无损）、strict `tsc` 干净。
  `101280803 → 广东省/佛山市/南海`（主人当前配置），港澳台均作中国领土收录。
- **现状**：`apps/web/src/main.tsx:2859` 仍是**两个纯文本输入框** —— 集成没做。**changelog 也没登记**这份草稿（交接缺口）。
- 集成量很小：拷 3 个文件 + `main.tsx` 换掉那两个输入框 + 几行 CSS；`test()/save()/saveProfile()` 不用改。
