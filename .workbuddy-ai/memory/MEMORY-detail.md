...
## 细则全文（2026-09-22 从 `MEMORY.md` 移入：时光机 / UI 硬规则 / 日历）

> 移入原因：`MEMORY.md` 会在注入时被截断，尾部三节等于「既读不到、又没有指针」。现在 `MEMORY.md` 只留每节的头条规则 + 本文件的标题，**细则全文在这里**。内容逐字保留，未改写。
...
- 差异行方图**上限 3 张**（`DIFF_PHOTO_LIMIT`），多的折成「还有 N 张」；`<img onError>` 落虚线占位；方图是 `loading="lazy"` ⇒ 断言 `naturalWidth > 0` 前先 `scrollIntoView` 再等 `complete`。
...
- **grid 自动排布会「只在部分记录上错」** → 几何恒定的元素一律显式定位；改单列时同步释放 `grid-column`。**不许有「移动的细周期图案」**（摩尔纹）—— **永不恢复移动的条纹幕布。**
...
- 天气天空两根轴：`category` 决定**画什么**、`phase` 只决定**色/光**，硬预算由 `verify-weather-art.mjs` 守。**补记条 = 安慰剂**（整条一个点击目标，子控件 `aria-hidden`）。
- **界面文字一律不可选中**（09-21 定案）：`body` + `button` 默认 `user-select:none`；白名单只有 ① 表单控件 ② 正文叶子（**清单只看 `styles.css` 顶部那块，共 10 个 —— 不抄，抄了会漂**）。**白名单只能放「承载文字的叶子」，绝不能放容器**（放 `.task-summary` 会连带 `<h2>` 与徽标一起可选）；**默认禁 + 白名单放** ⇒ 新加的按钮 / 菜单 / 徽标天生选不中，**别为单个元素加 `user-select:none`**。**例外**：只读**配置回显值** `.settings-ai-effective-item strong` 算内容，**只放值那一层**。验收 `.review/verify-selectability.mjs`（**只读贴 5199**，50 项；**不 spawn 隔离实例**，会假绿）—— 三层判据：全量扫描 / 点名对账 / **真手势拖蓝**。
- **改动前先勘察、先问**：设计/规则类先出方案等确认；主人说「**听你的**」才是授权点。

### 日历（09-21 定案，动手前先对表）

- **编辑模式只活在月视图**：`editable = editMode && mode === "month"`；**周卡没有小结行** ⇒ 周视图无铅笔、无小结字段，右键菜单只剩「周期 / 取消」。**初始模式是「周」**（刷新回周视图，别假设还在月视图）。
- **两种临时模式都不许常驻**：**不写 localStorage**；「点 `.calendar-section` 之外」「刷新」「离开日历视图」「切周视图」四条路都要退。**退出编辑模式 = 先存后退**（存失败就留在编辑模式，不弹框问）。
- **`hover` 展开的二级菜单，父行 `onClick` 不能再 toggle**（`mouseenter` 已展开）；父行 `onClick` 只负责展开，子菜单是父容器后代，移进去不触发 `mouseleave`。
- 右键「**编辑小结**」= **就地改那一天**（开编辑模式 + 光标落进那天）⇒ **当月每一天都有字段**，包括空白天。**齿轮面板里没有「编辑模式」开关**（已删），入口**只有铅笔那一个**。
...
### 为什么「我写了片名，却没变成观影信息」（2026-09-22 主人问）

**提示的判据只有一条**（`timeline.tsx:445`）：`recordText(record).includes("电影")` —— **正文里必须出现「电影」两个字**，字面匹配（`recordText = body.edited ?? body.original`）。

- 「重看《行尸走肉》第一季」这种**只写片名**的写法**永远不命中**。实测：150 条记录里含「电影」的 **0 条**、已关联影片的 **0 条** —— 这个入口从没被触发过。
- **已有记录想挂上**：编辑它、正文带上「电影」二字（如「重看电影《行尸走肉》第一季」）→ 保存后记录上就出现「电影」按钮 → 添加影片 → 搜片名 → 关联。**这是唯一入口**：`main.tsx:1289` 只把 `onMovieAttachToRecord` 传给 `<Timeline>`，而 composer 的 `/电影` 只能挂到**正在写的那条新记录**上。
- **未修的产品局限（等主人拍板）**：唯一触发词 + 字面匹配，对剧集 / 只写片名的写法不友好。可选改法：① 触发词扩到「看剧 / 追剧 / 剧集 / 观影」等；② 正文出现书名号 `《》` 就给提示；③ 每条日记都给入口（可能太吵）。

### 「斜杠菜单 / 自定义模块」的现状盘点（2026-09-22 主人问）

主人问「输入斜杠 → 下拉 → 方向键选模块（电影 / 电视剧 / 自定义，在设置里配）」有没有。**壳做好了，但只接了一个模块**：

| 他想要的 | 现状 |
| --- | --- |
| 打 `/` 出下拉 | ✅ `mention.tsx:141` `slashQueryAt` + `MentionBox` 渲染 |
| 方向键选 | ✅ `entity-forms.tsx:309-311`：↑↓ 循环、Enter 执行、Esc 关 |
| 按名字筛 | ✅ `mention.tsx:152` `slashSuggestions` 前缀匹配 `label` + `aliases` |
| 别名（`/观影`） | ✅ `ModuleCommand.aliases` 机制在（现在只有 `["观影"]`） |
| 多模块 | ⚠️ `ModuleCommand[]` 机制在，但 `enabledModuleCommands` **只返回电影一个** |
| 选中项决定行为 | ❌ `composer.tsx:254` 的 `onSlashCommand={() => setMoviePanelOpen(true)}` **忽略 command 参数** |
| 剧集 / 书等 | ❌ `EntityKind` 是硬编码枚举（`model.ts:6`：person/project/place/topic/movie），movie 另有一整套字段与校验 |
| 设置页自定义 | ❌ 模块页全硬编码（`app-meta.ts:57` + `settings-cards.tsx:757`） |

**更深的坑：识别只认电影。** `movie.ts:216` 打的是 TMDb **`/search/movie`** ⇒ 实测搜「行尸走肉」返回 9 部 **1936 / 1973 的老电影**，**2010 那部剧集根本不在候选里**。所以「想加个电视剧模块」的真实动机是这个，不是「模块数量不够」—— 就算把日记改成含「电影」二字、点开提示按钮，也挂不上那部剧。

**待定方案（主人未拍板，别自己动手）**：
- **A（小）**：触发判据认书名号 `《》` + 识别换 TMDb `/search/multi`（一次返回 movie + tv，带 `media_type`）。
- **B（中，我推荐）**：A + 实体加 `medium` 字段（`movie | tv`，**不动 `EntityKind` 枚举**）+ 候选带「电影 2010 / 剧集 2010」标签 + `/` 菜单列两项。
- **C（大）**：B + 设置页模块管理（自定义名字 / 别名 / 图标 / 数据源）—— 要做 schema 驱动的实体层，成本远大于收益。

**设计判断**：别让用户先选模块。他写日记是**先有内容**（「重看《行尸走肉》第一季」），内容里已经隐含了「这是部作品」；**让搜索结果自己标类型**，比让他替系统做判断自然得多。
